...
;; Defined in Xproto.h
;; Used by XGetErrorText
(define RequestCode
  (_enum '(X_CreateWindow             = 1
...
#;
 ...
 (defx11 XGetErrorText : _XDisplay-pointer ErrorCode _pointer (length : _int) -> _void)
 (define (XGetErrorText-user display code length)
   (let ((str (malloc _byte length)))
     (XGetErrorText display code str length)
     (define len 
       (for/or ([i length]) 
         (and (= (ptr-ref str _byte i) 0) i)))
     (make-sized-byte-string str len))) ; should str be freed at some point?
...
(defx11* XCirculateSubwindows      : _XDisplay-pointer Window _int -> _void)
(defx11* XCirculateSubwindowsDown  : _XDisplay-pointer Window -> _void)
(defx11* XCirculateSubwindowsUp    : _XDisplay-pointer Window -> _void)
